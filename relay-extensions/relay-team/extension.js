    ...
    view.webview.onDidReceiveMessage(async message => {
      try {
        ...
        if (message.type === 'runAgent') {
          ...
          const result = await api('start_agent', { request: { provider: message.provider, prompt: message.prompt, workspace, model: null, customCommand: message.customCommand || null } });
...
function teamHtml() {
  return shell(`<header><div><small>RELAY WORKSPACE</small><h2>Chat with Agent</h2></div><button id="setup" title="Configure Relay">?</button></header>
  <nav><button class="active" data-tab="agents">My agents <i id="count">0</i></button><button data-tab="chat">Rooms</button><button data-tab="skills">Skills</button></nav>
  <main id="content"><div class="empty"><div class="orb">R</div><h3>Your team is ready</h3><p>Chat with Codex, Claude Code, OpenCode, Azure, or your own agentic runtime. Every process, handoff, and decision stays visible.</p></div></main>
  <section class="composer"><div class="meta"><select id="provider"></select><span>workspace write</span></div><textarea id="prompt" placeholder="Ask an agent to build, fix, or explain…"></textarea><footer><button id="tests">Run tests</button><button id="room">Post to room</button><button id="send">Send to agent ?</button></footer></section>`, `
  const vscode=acquireVsCodeApi(),content=document.getElementById('content'),runs=new Map();let current='agents',skills=[],socket,shared={agents:{},tasks:{},messages:[],ownership:{},decisions:{}};
  const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  function connect(){socket=new WebSocket('ws://127.0.0.1:4173');socket.onmessage=e=>{const x=JSON.parse(e.data);if(x.type==='state.snapshot'){shared=x.payload;window.messages=shared.messages||[];Object.values(shared.agents||{}).forEach(a=>{const task=shared.tasks?.[a.taskId];runs.set(a.id,{...(runs.get(a.id)||{events:[]}),id:a.id,name:a.name,provider:a.provider,prompt:task?.title||a.detail||'Ready',status:a.status,taskId:a.taskId,files:a.files||[]})});render();return}if(x.type==='agent.updated'){const a=x.payload,task=shared.tasks?.[a.taskId];shared.agents[a.id]=a;runs.set(a.id,{...(runs.get(a.id)||{events:[]}),id:a.id,name:a.name,provider:a.provider,prompt:task?.title||a.detail||'Ready',status:a.status,taskId:a.taskId,files:a.files||[]});render()}if(x.type==='task.created'||x.type==='task.updated'){shared.tasks[x.payload.id]=x.payload;render()}if(x.type==='agent-event'){const ev=x.payload;let run=runs.get(ev.agentId)||{id:ev.agentId,name:'Workspace agent',prompt:'Running?',status:'working',events:[]};run.events=run.events||[];run.events.push(ev);if(ev.kind==='exit')run.status=ev.text.endsWith('0')?'done':'failed';runs.set(ev.agentId,run);render()}if(x.type==='message'){window.messages=window.messages||[];window.messages.push(x.payload);render()}};socket.onclose=()=>setTimeout(connect,1000)}
  function render(){document.getElementById('count').textContent=[...runs.values()].filter(x=>x.status==='working').length;if(current==='agents'){const list=[...runs.values()];content.innerHTML=list.length?list.map(r=>'<article><div class="agentTop"><div class="orb small">'+esc((r.provider||'R')[0].toUpperCase())+'</div><div><b>'+esc(r.name)+'</b><em class="'+r.status+'">'+r.status+'</em></div></div><p>'+esc(r.prompt)+'</p><div class="chips">'+(r.files||[]).map(f=>'<span>'+esc(f)+'</span>').join('')+'</div><div class="stream">'+r.events.slice(-10).map(e=>'<div class="'+e.kind+'"><i>›</i><span>'+esc(format(e.text))+'</span></div>').join('')+'</div></article>').join(''):'<div class="empty"><div class="orb">R</div><h3>No agents running</h3><p>Describe a task below to launch a real agent process.</p></div>'}else if(current==='skills'){content.innerHTML=skills.length?skills.map(s=>'<div class="skill"><b>'+esc(s.name)+'</b><em>'+esc(s.source)+'</em><p>'+esc(s.description)+'</p></div>').join(''):'<div class="empty"><p>No skills discovered.</p></div>'}else{const msgs=window.messages||[];content.innerHTML=msgs.length?msgs.map(m=>'<div class="msg"><b>'+esc(m.from)+' ? '+esc(m.to||'team')+'</b><em>'+esc(m.kind||'message')+'</em><p>'+esc(m.text)+'</p></div>').join(''):'<div class="empty"><p>Team and agent messages appear here live.</p></div>'}}
  ...
  document.getElementById('tests').onclick=()=>vscode.postMessage({type:'terminal',command:'npm test'});document.getElementById('room').onclick=()=>{const text=document.getElementById('prompt').value.trim();if(text&&socket?.readyState===1){socket.send(JSON.stringify({type:'message.send',payload:{to:'team',text}}));document.getElementById('prompt').value='';current='chat';document.querySelectorAll('nav button').forEach(x=>x.classList.toggle('active',x.dataset.tab==='chat'))}};document.getElementById('setup').onclick=()=>vscode.postMessage({type:'onboarding'});
